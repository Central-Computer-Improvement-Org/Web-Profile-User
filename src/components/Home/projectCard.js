...
const ProjectData = ({ projectsDataSecond, isLoading }) => {
    ...
    (type) => {
      ...
      if (type === 'left') {
        setPositionIndex((prevData) =>
          prevData === 0 ? projectsDataSecond.length - 1 : prevData - 1
        );
      } else if (type === 'right') {
        setPositionIndex((prevData) =>
          prevData === projectsDataSecond.length - 1 ? 0 : prevData + 1
        );
      }
    },
    [projectsDataSecond]
  ...
  if (!projectsDataSecond || isLoading) {
    return <Loading size="w-[40px] h-[40px] md:w-[100px] md:h-[100px]" />
  }
  ...
  return (
    <>
      {/* tampilan diatas tablet */}
      <div className="hidden w-full lg:block">
        <div className="w-full h-auto lg:h-[340px] xl:h-auto flex py-8 px-10 !mt-11 rounded-lg border-[3px] border-bluePallete-600 bg-transparent">
          {/* Project Thumbnail */}
          <div className="w-full h-[350px] max-w-[650px] max-h-[370px] lg:max-w-[467px] lg:max-h-[284px] rounded-lg object-cover">
            {isMovingData || isLoading || !projectsDataSecond ? (
              <Skeleton
                width="100%" 
                height="100%"
                style={{ borderRadius: '20px' }}
              />
            ) : projectsDataSecond[positionIndex]?.imageUri ? (
              <Image
                src={`${host}${projectsDataSecond[positionIndex].imageUri}`}
                alt="Thumbnail Project Central Computer Improvement"
                width={467}
                height={284}
                className="w-full h-[350px] max-w-[650px] max-h-[370px] lg:max-w-[467px] lg:max-h-[284px] rounded-lg object-cover"
              />
            ) : (
              <ImageNotFound
                className="w-full h-[350px] max-w-[650px] max-h-[370px] lg:max-w-[467px] lg:max-h-[284px] rounded-lg object-cover"
          ...
          {/* Project Content  */}
          <div className="flex flex-col justify-between mt-0 space-y-0 basis-full lg:basis-3/5 pl-7">
            <div className="flex flex-row items-center justify-start space-y-0">
              <div className="w-auto h-auto flex flex-row justify-normal items-center px-0 py-0 space-x-2 rounded-[10px] bg-transparent">
                {/* Project Icon  */}
                <div className="w-[50px] h-[50px]">
                  {isMovingData || isLoading || !projectsDataSecond ? (
                    <Skeleton circle={true} width="100%" height="100%" />
                  ) : projectsDataSecond[positionIndex]?.iconUri ? (
                    <Image
                      src={`${host}${projectsDataSecond[positionIndex].iconUri}`}
                    ...
                    <ImageNotFound className="w-[50px] h-[50px] object-contain" />
                  )}
                </div>
                {/* Project Title */}
                <div className="hidden sm:block ml-[10px]">
                  ...
                  ) : projectsDataSecond[positionIndex]?.name ? (
                    <p className="font-bold text-[18px] text-bluePallete-600 lg:text-black">
                      {projectsDataSecond[positionIndex].name}
                    </p>
                  ) : (
                    <TextNotFound className="font-bold text-[18px] text-transparent"></TextNotFound>
                  )}
                </div>
            ...
            <div className={`h-auto`}>
              ...
              ) : projectsDataSecond[positionIndex]?.description ? (
                <p
                  className={`font-medium !mt-0 text-[20px] leading-7 text-justify overflow-hidden text-black ${styles.projectsDesc}`}
                >
                  {projectsDataSecond[positionIndex].description}
                </p>
              ) : (
                <TextNotFound className="font-medium mt-0 text-[20px] leading-7 text-justify overflow-hidden text-transparent"></TextNotFound>
              )}
            </div>
            {/* Button */}
            <div
              className="flex flex-row flex-wrap items-center justify-start space-x-5"
            >
              {/* Button go to website */}
              <div
                onClick={() =>
                  window.open(projectsDataSecond[positionIndex].productionUri, '_blank')
                }
                className={`w-auto h-auto flex flex-row justify-between items-center py-2 xl:py-3 px-5 border-bluePallete-100 border-2 rounded-[15px] bg-bluePallete-900 hover:bg-white transition-colors text-bluePallete-100 hover:text-black duration-300 ease-in-out cursor-pointer ${styles.projectsContainerRepository}`}
              >
                <svg
                ...
                <a
                  ...
                  className="text-[15px] xl:text-[20px] font-semibold pl-[20px]"
                >
                  Go to Website
                </a>
              </div>
              ...
              <div
                onClick={() =>
                  window.open(projectsDataSecond[positionIndex].repositoryUri, '_blank')
                }
                className={`w-auto h-auto flex flex-row justify-between items-center space-x-5 py-2 xl:py-3 px-5 border-bluePallete-100 border-2 rounded-[15px] bg-bluePallete-900 hover:bg-white transition-colors text-bluePallete-100 hover:text-black border-none duration-300 ease-in-out cursor-pointer ${styles.projectsContainerRepository}`}
              >
                <svg
                ...
                <a
                  rel="noopener noreferrer"
                  className="text-[15px] xl:text-[20px] font-semibold"
                >
                  Check Repository
                </a>
              </div>
          ...
          <div className="absolute bottom-0 h-[85%] w-full bg-bluePallete-700 rounded-2xl"></div>
            ...
            {isMovingData || isLoading || !projectsDataSecond ? (
              ...
              <Image
                ...
                width={467}
                height={284}
                className={`relative z-10 w-[290px] h-[270px] sm:w-[480px] sm:h-[267px] md:w-full md:h-[350px] max-w-[650px] max-h-[370px] lg:max-w-[467px] lg:max-h-[284px] rounded-[20px] sm:rounded-lg object-cover ${styles.projectsThumbnailImage}`}
              />
              ) : (
                <ImageNotFound
            ...
            {/* Project Content  */}
            <div className="relative z-10 flex flex-col justify-between w-full gap-2 px-5 pt-3 pb-5">
              <div className="w-fit h-[39px] sm:h-auto flex flex-row justify-center sm:justify-normal items-center px-3 py-1 space-x-1 sm:space-x-2 rounded-[10px] bg-white">
                {/* Project Icon  */}
                <div className="w-[25px] h-[25px] sm:w-[35px] sm:h-[35px] lg:w-[50px]">
                    {isMovingData || isLoading || !projectsDataSecond ? (
                    <Skeleton circle={true} width="100%" height="100%" />
                    ) : projectsDataSecond[positionIndex]?.iconUri ? (
                    <Image
                        ...
                        width={50}
                        height={50}
                        className="w-[25px] h-[25px] sm:w-[35px] sm:h-[35px] object-contain"
                    />
                    ) : (
              ...
              <div className="h-auto py-2">
                {isMovingData || isLoading || !projectsDataSecond ? (
                <Skeleton count={3} className="my-1" />
                ) : projectsDataSecond[positionIndex]?.description ? (
                <p
                    className={`font-medium text-start text-[15px] leading-[20px] sm:leading-7 overflow-hidden text-white lg:text-black ${styles.projectsDesc}`}
                >
                    {projectsDataSecond[positionIndex].description}
                </p>
                ) : (
                <TextNotFound className="font-medium text-start text-[15px] !mt-4 leading-[20px] sm:leading-7 overflow-hidden text-transparent"></TextNotFound>
                )}
              </div>
              {/* Button link */}
              <div
                className="flex flex-row flex-wrap items-center justify-between space-x-0 md:space-x-5"
              >
                {/* Button go to website */}
                <div
                  onClick={() =>
                  ...
                  className="w-[168px] h-[39px] sm:w-auto sm:h-auto flex flex-row justify-start sm:justify-between items-center py-1 sm:py-2 px-2 sm:px-5 rounded-[5px] bg-white cursor-pointer"
                >
                <svg
                  className="hidden lg:block lg:w-[25px] lg:h-[25px]"
                  width="25"
                ...
                <div
                  ...
                  className={`w-[168px] h-[39px] sm:w-auto sm:h-auto flex flex-row justify-start sm:justify-between items-center space-x-1 sm:space-x-5 py-1 sm:py-2 px-2 sm:px-5 rounded-[5px] border-[2px] border-bluePallete-100 text-bluePallete-100 bg-bluePallete-900 ${styles.projectsContainerRepositoryMobile}`}
                >
                <svg
                  className="w-[20px] h-[20px] lg:w-[25px] lg:h-[25px]"
                  width="25"
              ...
              {/* Icon Arrow */}
              <div className="flex items-center justify-end w-full py-2 space-x-3 sm:space-x-4">
                  {/* Arrow Kiri */}
                  <svg
                    onClick={() => handleProjectArrow('left')}
