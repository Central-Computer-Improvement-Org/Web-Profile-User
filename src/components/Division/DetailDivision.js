...
export default function DetailDvision() {
   ...
   if (!divisionData) {
      return (
         <div className="flex items-center justify-center w-full h-screen py-52">
            <Loading
                  ...
                  {divisionData &&
                     divisionData.map((data) => (
                        <div key={data.id}>
                           <section
                              id="hero"
                              className="w-full h-auto flex flex-col space-y-[40px] pt-[100px] md:pt-[230px] mb-[50px] md:mb-[100px]"
                           >
                              <div className="flex items-center justify-between w-full h-auto">
                                 <div className="basis-3/5">
                                    {isLoading ? (
                                       ...
                                       <h1
                                          className={`font-black uppercase text-[25px] sm:text-[20px] md:text-[50px] lg:text-[70px] leading-tight text-secondPrimary ${styles.dvisionCardName}`}
                                       >
                                          {data.name}
                                       </h1>
                                    ) : (
                                       <TextNotFound className="font-black uppercase text-[25px] sm:text-[20px] leading-tight md:text-[50px] lg:text-[70px] text-transparent">
                                          Name Of Division
                                       </TextNotFound>
                                    )}
                                 </div>
                              ...
                              {isLoading ? (
                                 ...
                                 <p className="font-medium text-[10px] sm:text-[22px] lg:text-[30px] leading-0 md:leading-tight text-bluePallete-800">
                                    {data.description}
                                 </p>
                              ) : (
                                 <TextNotFound className="font-medium text-[14px] sm:text-[28px] lg:text-[40px] leading-0 md:leading-10 text-transparent">
                                    ...
                                    sesuai dengan divisi sediakan dengan memperhatikan beberapa struktur
                                    didalamnya.
                                 </TextNotFound>
                              )}
                           </section>
                           <section
                              id="team"
                              className="w-full h-auto flex flex-col mt-[50px] md:mt-[100px]"
                           >
