...
export default function Divisions() {
   ...
   useEffect(() => {
      request.get('/projects?id=PJT-20240508164541859997').then((v) => {
         console.info(v.data.data.contributors)
         if (v.data.code === 200 || v.data.code === 201) {
            setTeams(v.data.data?.contributors)
         } else {
            console.error(v.data.error);
         }
      }).catch((e) => {
         console.error(e);
      })
         ...
         {size.width >= 768 ? (
            ...
            <div className="flex justify-center mt-4">
               {Array.from({ length: totalDots }, (_, i) => (
                  <button
                     key={i}
                     className={`w-[10px] h-[10px] mx-[2px] rounded-full bg-gray-400 hover:bg-gray-500 ${i === currentIndex / teams.length ? "bg-blue-500" : ""
                        }`}
                     onClick={() => setCurrentIndex(i * teamsFilltered.length)}
                  />
               ))}
            </div>
         )
